...
| `LB-008` | High | Paper parity | Resolved in code (was: "Paper compatibility mode still uses ticker polling; deterministic replay is not yet the sole operational path"). Since `749468d` the only default Paper path is `MarketDataAdapter → Clock → PaperDriver → VersionBPaperRuntime`; the implicit legacy loop raises `LegacyPaperPathDisabled` (proven by `test_implicit_paper_no_longer_runs_outside_version_b`). Ticker polling survives only inside the retained legacy `PaperTradingExchange`, reachable solely via the explicit Python-only `allow_legacy_paper=True` override, which logs a critical warning and is never counted as Version B evidence. | Residual: a long Paper *operational validation* run on real cadence has not happened yet (that is `LB-001`, not a parity gap). | Deterministic replay, lifecycle, PnL reconciliation, and parity fixtures all pass at `56e061e`; remaining evidence folds into `LB-001` Paper operational validation. |
