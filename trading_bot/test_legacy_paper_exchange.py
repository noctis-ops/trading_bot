"""Legacy in-memory paper exchange — explicitly NOT Version B evidence.

``PaperTradingExchange`` predates Version B.  It keeps every position in a
Python dictionary, never writes to ``VersionBStore``, and is no longer on the
operational path: ``TradingBot`` refuses ``TRADING_MODE=paper`` through it
unless ``allow_legacy_paper=True`` is passed explicitly.

These tests are retained so the legacy exchange's own behaviour stays covered
while it still exists in the tree for the Live path.  Passing here proves
nothing about the Version B contract and must not be read as Paper readiness.
"""
...
class LegacyPaperExchangeTests(unittest.TestCase):
    """Legacy component coverage.  Not part of the Version B acceptance set."""
    ...
    def test_legacy_state_is_memory_only_and_does_not_survive_a_restart(self):
        """The reason this exchange is not on the operational path."""
        self.exchange.create_market_order("BTC/USDT", "buy", 1.0)
        self.assertIsNotNone(self.exchange.get_position_by_symbol("BTC/USDT"))
        restarted = PaperTradingExchange(initial_balance=1000.0)
        self.assertIsNone(restarted.get_position_by_symbol("BTC/USDT"))
        self.assertEqual(restarted.trade_history, [])
        self.assertEqual(len(restarted._lifecycles), 0)
