...
| `LB-002` | Critical | Safety / Execution | `No position without confirmed protection` is not yet proven against the real exchange acknowledgement/query path. | A filled position could remain without an exchange-confirmed SL. | Implement and test create → fetch/ack → durable confirmation; any failure blocks Live and triggers emergency handling. |
| `LB-003` | Critical | Persistence / Recovery | OrderManager and the live adapter are not hydrated/reconciled from `VersionBStore` after restart. | Duplicate orders, missed exits, or an unknown lifecycle can occur after a crash. | Restart/recovery fixture and live reconciliation evidence for every open `trade_id`, intent, order, event, and fill. |
| `LB-004` | Critical | Execution | Unknown order state, timeout, partial fill, cancel rejection, and exchange disconnect paths are not fully resolved. | The local lifecycle may diverge from exchange state and PnL. | Deterministic failure fixtures plus exchange-specific recovery policy; no guessing on unknown state. |
