...
class NoConfirmationExchange:
    """Models today's live adapter: it places orders but cannot confirm them."""
    ...
    def create_stop_loss_order(self, symbol, side, amount, stop_price):
        self.calls.append(("stop", symbol, side, amount, stop_price))
        # A non-empty create response is *not* confirmation.
        return {"id": "sl-1", "status": "new"}

    def create_take_profit_order(self, symbol, side, amount, stop_price):
        self.calls.append(("tp", symbol, side, amount, stop_price))
        return {"id": f"tp-{len(self.calls)}"}

    def create_market_order_close(self, symbol, side, amount):
        self.calls.append(("close", symbol, side, amount))
        return {"id": "close-1", "price": 100.0}
...
class OrderManagerSafetyTests(unittest.TestCase):
    ...
    def test_adapter_without_confirmation_fails_closed(self):
        """A create response alone must not be treated as confirmed protection.

        Previously the confirmation was skipped silently when the adapter had no
        ``record_protection``, which would have accepted an unprotected live
        position.  The contract requires failing closed instead.
        """
        exchange = NoConfirmationExchange()
        manager = OrderManager(exchange, FakeRiskManager())
        manager.MAX_RETRY_ATTEMPTS = 1
        result = manager.open_position(
            "BTC/USDT",
        ...
        self.assertFalse(result["success"])
        self.assertIn("cannot confirm protection", result["error"])
        self.assertNotIn("BTC/USDT", manager.open_positions)
        self.assertEqual([call[0] for call in exchange.calls].count("market"), 2, "emergency close attempted")
